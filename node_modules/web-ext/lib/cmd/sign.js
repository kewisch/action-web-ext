...
export default function sign({
  ...
  return withTempDir(async function (tmpDir) {
    ...
    if (idFromSourceDir && !id) {
      throw new UsageError('Cannot use previously auto-generated extension ID ' + `${idFromSourceDir} - This extension ID must be specified in the manifest.json file.`);
    }
    if (!id) {
      // We only auto-generate add-on IDs for MV2 add-ons on AMO.
      if (manifestData.manifest_version !== 2) {
        throw new UsageError('An extension ID must be specified in the manifest.json file.');
      }
      log.warn('No extension ID specified (it will be auto-generated the first time)');
    }
    if (!channel) {
