module.exports = { domain:"messages",
  locale_data:{ messages:{ "":{ domain:"messages",
        plural_forms:"nplurals=2; plural=(n != 1);",
        lang:"si" },
      "Validation Summary:":[ "වලංගුකරණ සාරාංශය:" ],
      Code:[ "කේතය" ],
      Message:[ "පණිවිඩය" ],
      Description:[ "සවිස්තරය" ],
      File:[ "ගොනුව" ],
      Line:[ "පේළිය" ],
      Column:[ "තීරුව" ],
      "Invalid manifest version range requested: --min-manifest-version (currently set to %(minManifestVersion)s) should not be greater than --max-manifest-version (currently set to %(maxManifestVersion)s).":[ "" ],
      "Your FTL is not valid.":[ "ඔබගේ FTL වලංගු නොවේ." ],
      "Your FTL file could not be parsed.":[ "ඔබගේ FTL ගොනුව විග්‍රහ කළ නොහැකිය." ],
      "Remote scripts are not allowed as per the Add-on Policies.":[ "එක්කහු ප්‍රතිපත්තිවලට අනුව දුරස්ථ අත්පත් සඳහා ඉඩ නොදේ." ],
      "Please include all scripts in the add-on. For more information, refer to https://mzl.la/2uEOkYp.":[ "" ],
      "Inline scripts blocked by default":[ "" ],
      "Default CSP rules prevent inline JavaScript from running (https://mzl.la/2pn32nd).":[ "" ],
      "Unadvised 3rd-party JS library":[ "" ],
      "Your add-on uses a JavaScript library we do not recommend. Read more: https://bit.ly/1TRIyZY":[ "" ],
      "Known JS library detected":[ "දන්නා JS එකතුවක් අනාවරණය විය" ],
      "JavaScript libraries are discouraged for simple add-ons, but are generally accepted.":[ "" ],
      "Due to both security and performance concerns, this may not be set using dynamic values which have not been adequately sanitized. This can lead to security issues or fairly serious performance degradation.":[ "" ],
      "{{api}} is not supported":[ "{{api}} සහාය නොදක්වයි" ],
      "This API has not been implemented by Firefox.":[ "ෆයර්ෆොක්ස් සඳහා මෙම යෙ.ක්‍ර.මු. ක්‍රියාවට නංවා නැත." ],
      "\"{{api}}\" has been removed in Manifest Version 3 (`manifest_version` property)":[ "ව්‍යක්ත අනුවාදය 3 (`manifest_version` ගුණාංගය) හි \"{{api}}\" ඉවත් කර ඇත" ],
      "{{api}} is deprecated":[ "{{api}} අත්හැර ඇත" ],
      "This API has been deprecated by Firefox.":[ "ෆයර්ෆොක්ස් මගින් මෙම යෙ.ක්‍ර.මු. අත්හැර ඇත." ],
      "Content script file could not be found.":[ "අන්තර්ගත අත්පත් ගොනුව හමු නොවිණි." ],
      "\"{{api}}\" is deprecated or unimplemented":[ "\"{{api}}\" අත්හැර හෝ ක්‍රියාවට නංවා නැත" ],
      "Content script file could not be found":[ "අන්තර්ගත අත්පත් ගොනුව හමු නොවිණි" ],
      "\"%(api)s\" can cause issues when loaded temporarily":[ "" ],
      "This API can cause issues when loaded temporarily using about:debugging in Firefox unless you specify \"browser_specific_settings.gecko.id\" in the manifest. Please see: https://mzl.la/2hizK4a for more.":[ "" ],
      "{{api}} is not supported in Firefox version {{minVersion}}":[ "ෆයර්ෆොක්ස් අනුවාදය {{minVersion}} සඳහා {{api}} සහාය නොදක්වයි" ],
      "This API is not implemented by the given minimum Firefox version":[ "මෙම යෙ.ක්‍ර.මු. දී ඇති අවම ෆයර්ෆොක්ස් අනුවාදය ක්‍රියාවට නංවා නැත" ],
      "{{api}} is not supported in Firefox for Android version {{minVersion}}":[ "ඇන්ඩ්‍රොයිඩ් සඳහා ෆයර්ෆොක්ස් අනුවාදය {{minVersion}} හි {{api}} සහාය නොදක්වයි" ],
      "This API is not implemented by the given minimum Firefox for Android version":[ "මෙම යෙ.ක්‍ර.මු. දී ඇති අවම ඇන්ඩ්‍රොයිඩ් සඳහා ෆයර්ෆොක්ස් අනුවාදය ක්‍රියාවට නංවා නැත" ],
      "Content script file name should not be empty.":[ "අන්තර්ගත අත්පත් ගොනුවේ නම හිස් නොවිය යුතුය." ],
      "Content script file name should not be empty":[ "අන්තර්ගත අත්පත් ගොනුවේ නම හිස් නොවිය යුතුය" ],
      "\"%(method)s\" called with a non-literal uri":[ "" ],
      "Calling \"%(method)s\" with variable parameters can result in potential security vulnerabilities if the variable contains a remote URI. Consider using 'window.open' with the 'chrome=no' flag.":[ "" ],
      "\"%(method)s\" called with non-local URI":[ "" ],
      "Calling \"%(method)s\" with a non-local URI will result in the dialog being opened with chrome privileges.":[ "" ],
      "JavaScript syntax error":[ "ජාවාස්ක්‍රිප්ට් පද ගැළපුම් දෝෂයකි" ],
      "There is a JavaScript syntax error in your code, which might be related to some experimental JavaScript features that aren't an official part of the language specification and therefore not supported yet. The validation cannot continue on this file.":[ "" ],
      "Evaluation of strings as code can lead to security vulnerabilities and performance issues, even in the most innocuous of circumstances. Please avoid using `eval` and the `Function` constructor when at all possible.":[ "" ],
      "setTimeout, setInterval and execScript functions should be called only with function expressions as their first argument":[ "" ],
      "Unexpected global passed as an argument":[ "" ],
      "Passing a global as an argument is not recommended. Please make this a var instead.":[ "" ],
      "Use of document.write strongly discouraged.":[ "document.write භාවිතය නිර්දේශ නොකෙරේ." ],
      "document.write will fail in many circumstances when used in extensions, and has potentially severe security repercussions when used improperly. Therefore, it should not be used.":[ "" ],
      "Banned 3rd-party JS library":[ "තහනම් තෙවන පාර්ශ්ව JS එකතුවකි" ],
      "Your add-on uses a JavaScript library we consider unsafe. Read more: https://bit.ly/1TRIyZY":[ "" ],
      "Your JSON contains block comments.":[ "ඔබගේ JSON හි අවහිර අදහස් අඩංගු වේ." ],
      "Only line comments (comments beginning with \"//\") are allowed in JSON files. Please remove block comments (comments beginning with \"/*\")":[ "" ],
      "Duplicate keys are not allowed in JSON files.":[ "JSON ගොනු තුළ යතුරු අනුපිටපත් තිබීමට නොහැකිය." ],
      "Duplicate key found in JSON file.":[ "" ],
      "Your JSON is not valid.":[ "ඔබගේ JSON වලංගු නොවේ." ],
      "Your JSON file could not be parsed.":[ "ඔබගේ JSON ගොනුව විග්‍රහ කළ නොහැකිය." ],
      "Reserved filename found.":[ "රක්‍ෂිත ගොනු නාමයක් හමු විය." ],
      "Files whose names are reserved have been found in the add-on. Please refrain from using them and rename your files.":[ "" ],
      "The package is invalid. It may contain entries using invalid characters, as an example using '\\' as a path separator is not allowed in Firefox. Try to recreate your add-on package (ZIP) and make sure all entries are using '/' as the path separator.":[ "" ],
      "We were unable to decompress the zip file.":[ "zip ගොනුව දිග හැරීමට නොහැකි විය." ],
      "manifest.json was not found":[ "manifest.json හමු නොවිණි" ],
      "No manifest.json was found at the root of the extension. The package file must be a ZIP of the extension's files themselves, not of the containing directory. See: https://mzl.la/2r2McKv for more on packaging.":[ "" ],
      "File is too large to parse.":[ "විග්‍රහ කිරීමට ගොනුව විශාල වැඩිය." ],
      "This file is not binary and is too large to parse. Files larger than %(maxFileSizeToParseMB)sMB will not be parsed. Consider moving large lists of data out of JavaScript files and into JSON files, or splitting very large files into smaller ones.":[ "" ],
      "Hidden file flagged":[ "" ],
      "Hidden files complicate the review process and can contain sensitive information about the system that generated the add-on. Please modify the packaging process so that these files aren't included.":[ "" ],
      "Package contains duplicate entries":[ "ඇසුරුමෙහි අනුපිටපත් නිවේශිත තිබේ" ],
      "Flagged filename found":[ "" ],
      "Files were found that are either unnecessary or have been included unintentionally. They should be removed.":[ "" ],
      "The package contains multiple entries with the same name. This practice has been banned. Try unzipping and re-zipping your add-on package and try again.":[ "" ],
      "Flagged file extensions found":[ "" ],
      "Flagged file type found":[ "" ],
      "Files whose names end with flagged extensions have been found in the add-on. The extension of these files are flagged because they usually identify binary components. Please see https://bit.ly/review-policy for more information on the binary content review process.":[ "" ],
      "Package already signed":[ "ඇසුරුම දැනටමත් අත්සන් කර ඇත" ],
      "Add-ons which are already signed will be re-signed when published on AMO. This will replace any existing signatures on the add-on.":[ "" ],
      "Firefox add-ons are not allowed to run coin miners.":[ "ෆයර්ෆොක්ස් එක්කහු මගින් කාසි කැණීමට ඉඩ නොදේ." ],
      "We do not allow coinminer scripts to be run inside WebExtensions. See https://github.com/mozilla/addons-linter/issues/1643 for more details.":[ "" ],
      "String name is reserved for a predefined message":[ "තන්තු නාමය නිශ්චිත පණිවිඩයක් සඳහා වෙන් කර ඇත" ],
      "String names starting with @@ get translated to built-in constants (https://mzl.la/2BL9ZjE).":[ "" ],
      "String name should only contain alpha-numeric characters, _ and @ (https://mzl.la/2Eztyi5).":[ "" ],
      "Placeholder for message is missing":[ "" ],
      "A placeholder used in the message is not defined.":[ "" ],
      "Placeholder name contains invalid characters":[ "" ],
      "Placeholder name should only contain alpha-numeric characters, _ and @ (https://mzl.la/2ExbYez).":[ "" ],
      "Placeholder is missing the content property":[ "" ],
      "A placeholder needs a content property defining the replacement of it (https://mzl.la/2DT1MQd)":[ "" ],
      "Translation string is missing the message property":[ "පරිවර්තන තන්තුවට පණිවිඩ ගුණාංගය මඟහැරී ඇත" ],
      "No \"message\" message property is set for a string (https://mzl.la/2DSBTjA).":[ "තන්තුවක් සඳහා \"message\" පණිවිඩ ගුණාංගයක් සකසා නැත (https://mzl.la/2DSBTjA)." ],
      "The field is required.":[ "ක්‍ෂේත්‍රය අවශ්‍යයි." ],
      "The permission type is unsupported.":[ "සහාය නොදක්වන අවසර වර්ගයකි." ],
      "See https://mzl.la/1R1n1t0 (MDN Docs) for more information.":[ "තව විස්තර සඳහා https://mzl.la/1R1n1t0 (MDN ප්‍රලේඛනය) බලන්න." ],
      "See https://mzl.la/1ZOhoEN (MDN Docs) for more information.":[ "තව විස්තර සඳහා https://mzl.la/1ZOhoEN (MDN ප්‍රලේඛනය) බලන්න." ],
      "See https://mzl.la/2Qn0fWC (MDN Docs) for more information.":[ "තව විස්තර සඳහා https://mzl.la/2Qn0fWC (MDN ප්‍රලේඛනය) බලන්න." ],
      "See https://mzl.la/3Woeqv4 (MDN Docs) for more information.":[ "" ],
      "Unknown permission.":[ "නොදන්නා අවසරයකි." ],
      "%(instancePath)s: the following privileged permissions are only allowed in privileged extensions: %(privilegedPermissions)s.":[ "" ],
      "Invalid host permission.":[ "සත්කාරක අවසරය වලංගු නොවේ." ],
      "Invalid install origin.":[ "ස්ථාපන ප්‍රභවය වලංගු නොවේ." ],
      "Invalid install origin. A valid origin has - only - a scheme, hostname and optional port. See https://mzl.la/3TEbqbE (MDN Docs) for more information.":[ "" ],
      "\"manifest_version\" in the manifest.json is not a valid value":[ "manifest.json හි \"manifest_version\" වලංගු අගයක් නොවේ" ],
      "See https://mzl.la/20PenXl (MDN Docs) for more information.":[ "තව විස්තර සඳහා https://mzl.la/20PenXl (MDN ප්‍රලේඛනය) බලන්න." ],
      "The field is invalid.":[ "ක්‍ෂේත්‍රය වලංගු නොවේ." ],
      "\"%(property)s\" allows remote code execution in manifest.json":[ "" ],
      "A custom \"%(property)s\" needs additional review.":[ "" ],
      "\"%(property)s\" allows 'eval', which has strong security and performance implications.":[ "" ],
      "In most cases the same result can be achieved differently, therefore it is generally prohibited":[ "" ],
      "The \"name\" property must be a string with no leading/trailing whitespaces.":[ "" ],
      "See http://mzl.la/1STmr48 (MDN Docs) for more information.":[ "තව විස්තර සඳහා http://mzl.la/1STmr48 (MDN ප්‍රලේඛනය) බලන්න." ],
      "\"update_url\" is not allowed.":[ "\"update_url\" සඳහා ඉඩ නොදේ." ],
      "\"applications.gecko.update_url\" or \"browser_specific_settings.gecko.update_url\" are not allowed for Mozilla-hosted add-ons.":[ "" ],
      "The \"update_url\" property is not used by Firefox.":[ "\"update_url\" ගුණාංගය ෆයර්ෆොක්ස් සඳහා භාවිතා නොකෙරේ." ],
      "The \"update_url\" is not used by Firefox in the root of a manifest; your add-on will be updated via the Add-ons site and not your \"update_url\". See: https://mzl.la/25zqk4O":[ "" ],
      "\"strict_max_version\" not required.":[ "\"strict_max_version\" අවශ්‍ය නොවේ." ],
      "\"strict_max_version\" shouldn't be used unless the add-on is expected not to work with future versions of Firefox.":[ "" ],
      "No \"%(property)s\" property found in manifest.json":[ "" ],
      "\"%(property)s\" is required":[ "\"%(property)s\" වුවමනාය" ],
      "An icon defined in the manifest could not be found in the package.":[ "ව්‍යක්තයෙහි දක්වා තිබෙන නිරූපකයක් ඇසුරුම තුළ හමු නොවිණි." ],
      "Icon could not be found at \"%(path)s\".":[ "\"%(path)s\" හි නිරූපකය හමු නොවිණි." ],
      "A background script defined in the manifest could not be found.":[ "ව්‍යක්තයෙහි දක්වා තිබෙන පසුබිම් අත්පතක් හමු නොවිණි." ],
      "A background page defined in the manifest could not be found.":[ "ව්‍යක්තයෙහි දක්වා තිබෙන පසුබිම් පිටුවක් හමු නොවිණි." ],
      "Background script could not be found at \"%(path)s\".":[ "\"%(path)s\" හි පසුබිම් අත්පත හමු නොවිණි." ],
      "Background page could not be found at \"%(path)s\".":[ "\"%(path)s\" හි පසුබිම් පිටුව හමු නොවිණි." ],
      "\"%(fieldName)s\" is ignored for non-privileged add-ons.":[ "" ],
      "A content script defined in the manifest could not be found.":[ "ව්‍යක්තයෙහි දක්වා තිබෙන අන්තර්ගත අත්පතක් හමු නොවිණි." ],
      "A content script css file defined in the manifest could not be found.":[ "ව්‍යක්තයෙහි දක්වා තිබෙන අන්තර්ගත අත්පතක css ගොනුවක් හමු නොවිණි." ],
      "Content script defined in the manifest could not be found at \"%(path)s\".":[ "ව්‍යක්තයෙහි දක්වා තිබෙන අන්තර්ගත අත්පත, \"%(path)s\" හි හමු නොවිණි." ],
      "Content script css file defined in the manifest could not be found at \"%(path)s\".":[ "ව්‍යක්තයෙහි දක්වා තිබෙන අන්තර්ගත අත්පතක css ගොනුවක්, \"%(path)s\" හි හමු නොවිණි." ],
      "A dictionary file defined in the manifest could not be found.":[ "ව්‍යක්තයෙහි දක්වා තිබෙන ශබ්දකෝෂ ගොනුවක් හමු නොවිණි." ],
      "\"%(fieldName)s\" manifest field is only used for privileged and temporarily installed extensions.":[ "" ],
      "Dictionary file defined in the manifest could not be found at \"%(path)s\".":[ "ව්‍යක්තයෙහි දක්වා තිබෙන ශබ්දකෝෂ ගොනුව, \"%(path)s\" හි හමු නොවිණි." ],
      "The manifest contains multiple dictionaries.":[ "ව්‍යක්තයෙහි ශබ්දකෝෂ කිහිපයක් අඩංගුය." ],
      "Multiple dictionaries were defined in the manifest, which is unsupported.":[ "ව්‍යක්තයෙහි ශබ්දකෝෂ කිහිපයක් නිරූපිතයි, එයට සහාය නොදක්වයි." ],
      "The manifest contains a dictionaries object, but it is empty.":[ "ව්‍යක්තයෙහි ශබ්දකෝෂ වස්තුවක් ඇත, නමුත් එය හිස් ය." ],
      "A dictionaries object was defined in the manifest, but it was empty.":[ "ශබ්දකෝෂ වස්තුවක් ව්‍යක්තයෙහි දක්වා ඇත, නමුත් එය හිස් ය." ],
      "The manifest contains a dictionary but no id property.":[ "ව්‍යක්තයෙහි ශබ්දකෝෂයක් අඩංගු නමුත් හැඳු. ගුණාංගය නැත." ],
      "A dictionary was found in the manifest, but there was no id set.":[ "ව්‍යක්තයෙහි ශබ්දකෝෂයක් හමු වූ නමුත් හැඳු. සකසා නැත." ],
      "Forbidden content found in add-on.":[ "එක්කහුව තුළ තහනම් අන්තර්ගතයක් හමු විය." ],
      "This add-on contains forbidden content.":[ "මෙම එක්කහුවෙහි තහනම් අන්තර්ගතයක් අඩංගුය." ],
      "Please refer to https://github.com/mozilla-extensions/xpi-manifest to learn more about privileged extensions and signing.":[ "" ],
      "Icons must be square.":[ "නිරූපකය සතරැස් විය යුතුය." ],
      "Icon at \"%(path)s\" must be square.":[ "\"%(path)s\" හි නිරූපකය සතරැස් විය යුතුය." ],
      "The size of the icon does not match the manifest.":[ "නිරූපකයේ ප්‍රමාණය ව්‍යක්තයට නොගැළපේ." ],
      "Expected icon at \"%(path)s\" to be %(expected)d pixels wide but was %(actual)d.":[ "" ],
      "Corrupt image file":[ "හානි වූ රූප ගොනුවකි" ],
      "Expected icon file at \"%(path)s\" is corrupted":[ "\"%(path)s\" හි අපේක්‍ෂිත නිරූපක ගොනුව හානි වී ඇත" ],
      "This property has been deprecated.":[ "මෙම ගුණාංගය අත්හැර ඇත." ],
      "This theme LWT alias has been removed in Firefox 70.":[ "ෆයර්ෆොක්ස් 70 හි මෙම තේමාවේ LWT අපනාමය ඉවත් කර ඇත." ],
      "See https://mzl.la/2T11Lkc (MDN Docs) for more information.":[ "තව විස්තර සඳහා https://mzl.la/2T11Lkc (MDN ප්‍රලේඛනය) බලන්න." ],
      "Theme image for \"%(type)s\" could not be found at \"%(path)s\"":[ "\"%(type)s\" සඳහා තේමා රූපය \"%(path)s\" හි සොයාගත නොහැකි විය" ],
      "Corrupted theme image file":[ "හානි වූ තේමා රූප ගොනුවකි" ],
      "Theme image file at \"%(path)s\" is corrupted":[ "\"%(path)s\" හි තේමා රූප ගොනුව හානි වී ඇත" ],
      "\"%(fieldName)s\" is not supported in manifest versions %(versionRange)s.":[ "" ],
      "Theme image file has an unsupported file extension":[ "තේමා රූප ගොනුවේ සහාය නොදක්වන ගොනු දිගුවක් ඇත" ],
      "Theme image file at \"%(path)s\" has an unsupported file extension":[ "\"%(path)s\" හි තේමා රූප ගොනුව සඳහා සහාය නොදක්වන ගොනු දිගුවක් ඇත" ],
      "Theme image file has an unsupported mime type":[ "තේමා රූප ගොනුවේ සහාය නොදක්වන mime වර්ගයක් ඇත" ],
      "Theme image file at \"%(path)s\" has the unsupported mime type \"%(mime)s\"":[ "\"%(path)s\" හි තේමා රූප ගොනුවෙහි සහාය නොදක්වන \"%(mime)s\" mime වර්ගය ඇත" ],
      "\"%(fieldName)s\" is not supported.":[ "" ],
      "Theme image file mime type does not match its file extension":[ "තේමා රූප ගොනුවේ mime වර්ගය එහි ගොනු දිගුව සමඟ නොගැළපේ" ],
      "Theme image file extension at \"%(path)s\" does not match its actual mime type \"%(mime)s\"":[ "\"%(path)s\" හි තේමා රූප ගොනු දිගුව එහි සැබෑ \"%(mime)s\" mime වර්ගයට නොගැළපේ" ],
      "The \"default_locale\" is missing localizations.":[ "\"default_locale\" හි දේශීයකරණ මඟහැරී ඇත." ],
      "The \"default_locale\" value is specified in the manifest, but no matching \"messages.json\" in the \"_locales\" directory exists. See: https://mzl.la/2hjcaEE":[ "" ],
      "The \"default_locale\" is missing but \"_locales\" exist.":[ "\"default_locale\" මඟහැරී ඇත නමුත් \"_locales\" පවතී." ],
      "The \"default_locale\" value is not specifed in the manifest, but a \"_locales\" directory exists. See: https://mzl.la/2hjcaEE":[ "" ],
      "Unsupported image extension":[ "සහාය නොදක්වන රූප දිගුවකි" ],
      "Icons should be one of JPG/JPEG, WebP, GIF, PNG or SVG.":[ "නිරූපක JPG/JPEG, WebP, GIF, PNG හෝ SVG වලින් එකක් විය යුතුය." ],
      "\"applications\" property overridden by \"browser_specific_settings\" property":[ "\"යෙදුම්\" ගුණාංගය \"browser_specific_settings\" මගින් අභිබවයි" ],
      "The \"applications\" property is being ignored because it is superseded by the \"browser_specific_settings\" property which is also defined in your manifest. Consider removing applications.":[ "" ],
      "Empty language directory":[ "හිස් භාෂා නාමාවලිය" ],
      "messages.json file missing in \"%(path)s\"":[ "\"%(path)s\" හි messages.json ගොනුව මඟහැරී ඇත" ],
      "Manifest key not supported by the specified minimum Firefox version":[ "නිරූපිත ඇන්ඩ්‍රොයිඩ් සඳහා ෆයර්ෆොක්ස් අවම අනුවාදයට ව්‍යක්ත යතුර සහාය නොදක්වයි" ],
      "\"strict_min_version\" requires Firefox %(minVersion)s, which was released before version %(versionAdded)s introduced support for \"%(key)s\".":[ "" ],
      "Permission not supported by the specified minimum Firefox version":[ "නිරූපිත ඇන්ඩ්‍රොයිඩ් සඳහා ෆයර්ෆොක්ස් අවම අනුවාදයට අවසරය සහාය නොදක්වයි" ],
      "%(instancePath)s: privileged manifest fields are only allowed in privileged extensions.":[ "" ],
      "Manifest key not supported by the specified minimum Firefox for Android version":[ "නිරූපිත ඇන්ඩ්‍රොයිඩ් සඳහා ෆයර්ෆොක්ස් අවම අනුවාදයට ව්‍යක්ත යතුර සහාය නොදක්වයි" ],
      "\"strict_min_version\" requires Firefox for Android %(minVersion)s, which was released before version %(versionAdded)s introduced support for \"%(key)s\".":[ "" ],
      "Permission not supported by the specified minimum Firefox for Android version":[ "නිරූපිත ඇන්ඩ්‍රොයිඩ් සඳහා ෆයර්ෆොක්ස් අවම අනුවාදයට අවසරය සහාය නොදක්වයි" ],
      "Linking to \"addons.mozilla.org\" is not allowed":[ "\"addons.mozilla.org\" වෙත සබැඳීමට ඉඩ නොදේ" ],
      "Links directing to \"addons.mozilla.org\" are not allowed to be used for homepage":[ "මුල් පිටුව සඳහා \"addons.mozilla.org\" වෙත යොමු කරන සබැඳි භාවිතා කිරීමට ඉඩ නොදේ" ],
      "The \"%(permission)s\" permission requires \"strict_min_version\" to be set to \"%(minFirefoxVersion)s\" or above":[ "" ],
      "The \"%(permission)s\" permission requires \"strict_min_version\" to be set to \"%(minFirefoxVersion)s\" or above. Please update your manifest.json version to specify a minimum Firefox version.":[ "" ],
      "The extension ID is required in Manifest Version 3 and above.":[ "අනුවාදය 3 හා නව ව්‍යක්ත සඳහා දිගු හැඳු. අවශ්‍ය වේ." ],
      "See https://mzl.la/3PLZYdo for more information.":[ "වැඩි විස්තර සඳහා https://mzl.la/3PLZYdo බලන්න." ],
      "%(instancePath)s: Privileged extensions should declare privileged permissions.":[ "" ],
      "This extension does not declare any privileged permission. It does not need to be signed with the privileged certificate. Please upload it directly to https://addons.mozilla.org/.":[ "" ],
      "%(instancePath)s: The \"mozillaAddons\" permission is required for privileged extensions.":[ "" ],
      "%(instancePath)s: The \"mozillaAddons\" permission is required for extensions that include privileged manifest fields.":[ "" ],
      "This extension does not include the \"mozillaAddons\" permission, which is required for privileged extensions.":[ "" ],
      "Cannot use actions in hidden add-ons.":[ "" ],
      "The hidden and browser_action/page_action (or action in Manifest Version 3 and above) properties are mutually exclusive.":[ "" ],
      "Use \"browser_specific_settings\" instead of \"applications\".":[ "" ],
      "The \"applications\" property in the manifest is deprecated and will no longer be accepted in Manifest Version 3 and above.":[ "" ],
      "\"applications\" is no longer allowed in Manifest Version 3 and above.":[ "" ],
      "The \"applications\" property in the manifest is no longer allowed in Manifest Version 3 and above. Use \"browser_specific_settings\" instead.":[ "" ],
      "The version string should be simplified because it won't be compatible with Manifest Version 3 and above.":[ "" ],
      "The version should be a string with 1 to 4 numbers separated with dots. Each number should have up to 9 digits and leading zeros will no longer be allowed. Letters will no longer be allowed either. See https://mzl.la/3h3mCRu (MDN Docs) for more information.":[ "" ],
      "The version string should be simplified.":[ "" ],
      "The version should be a string with 1 to 4 numbers separated with dots. Each number should have up to 9 digits and leading zeros are not allowed. Letters are no longer allowed. See https://mzl.la/3h3mCRu (MDN Docs) for more information.":[ "" ],
      "incognito \"split\" is unsupported.":[ "" ],
      "The incognito \"split\" value is unsupported and will be treated as \"not_allowed\" in Firefox. Remove the key from the manifest file, if your extension is compatible with Firefox's private browsing mode.":[ "" ],
      "The \"admin_install_only\" property is reserved.":[ "" ],
      "The \"admin_install_only\" property is reserved and can only be used in enterprise add-ons.":[ "" ],
      "/%(fieldName)s: \"%(permissionName)s\" is not supported in manifest versions %(versionRange)s.":[ "" ],
      "\"/browser_specific_settings/gecko/admin_install_only\" property must be set to \"true\".":[ "" ],
      "The \"admin_install_only\" property must be set to \"true\" in an enterprise add-on.":[ "" ],
      "/%(fieldName)s: \"%(permissionName)s\" is not supported.":[ "" ] } } }