...
function formatIssuesWithRemediation(vulns, remediationInfo, options) {
    ...
    if (remediationInfo.pin && Object.keys(remediationInfo.pin).length) {
        ...
        Object.keys(remediationInfo.pin).forEach((name) => remediationInfo.pin[name].issues.forEach((vid) => allVulnIds.add(vid)));
...
function constructLicenseText(basicLicenseInfo, testOptions) {
    ...
    for (const id of Object.keys(basicLicenseInfo)) {
        const licenseText = formatIssue(id, basicLicenseInfo[id].title, basicLicenseInfo[id].severity, basicLicenseInfo[id].isNew, basicLicenseInfo[id].legalInstructions, `${basicLicenseInfo[id].name}@${basicLicenseInfo[id].version}`, basicLicenseInfo[id].paths, testOptions);
...
function constructPatchesText(patches, basicVulnInfo, testOptions) {
    ...
    for (const id of Object.keys(patches)) {
        ...
        const thisPatchFixes = formatIssue(id, basicVulnInfo[id].title, basicVulnInfo[id].severity, basicVulnInfo[id].isNew, undefined, `${basicVulnInfo[id].name}@${basicVulnInfo[id].version}`, basicVulnInfo[id].paths, testOptions);
...
function thisUpgradeFixes(vulnIds, basicVulnInfo, testOptions) {
    return vulnIds
        ...
        .map((id) => formatIssue(id, basicVulnInfo[id].title, basicVulnInfo[id].severity, basicVulnInfo[id].isNew, undefined, `${basicVulnInfo[id].name}@${basicVulnInfo[id].version}`, basicVulnInfo[id].paths, testOptions))
        .join('\n');
}
function processUpgrades(sink, upgradesByDep, deps, basicVulnInfo, testOptions) {
    for (const dep of deps) {
        ...
        const vulnIds = data.vulns || data.issues;
...
function constructUnfixableText(unresolved, basicVulnInfo, testOptions) {
    ...
    for (const issue of unresolved) {
        ...
        unfixableIssuesTextArray
            .push(formatIssue(issue.id, issue.title, issue.severity, issue.isNew, undefined, `${issue.packageName}@${issue.version}`, basicVulnInfo[issue.id].paths, testOptions) + `${extraInfo}`);
    }
    return unfixableIssuesTextArray;
}
function printPath(path) {
    return path.slice(1).map((name, i) => chalk_1.default.cyan(name)).join(' > ');
}
function formatIssue(id, title, severity, isNew, legalInstructions, vulnerableModule, paths, testOptions) {
