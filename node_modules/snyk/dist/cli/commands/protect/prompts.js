...
function generatePrompt(vulns, policy, prefix, options) {
    ...
    let prompts = vulns.map((vuln, i) => {
        ...
        if (group) {
            infoLink += chalk_1.default.underline('/package/npm/' + group.affected.name + '/' +
                group.affected.version);
            const joiningText = group.patch ? 'in' : 'via';
            const issues = vuln.type === 'license' ? 'issues' : 'vulnerabilities';
            messageIntro = util_1.format('✗ %s %s %s introduced %s %s', group.count, severity, issues, joiningText, group.affected.full);
            messageIntro = createSeverityBasedIssueHeading(messageIntro, vuln.severity);
        }
        else {
            infoLink += chalk_1.default.underline('/vuln/' + vuln.id);
            messageIntro = util_1.format('✗ %s severity %s found in %s, introduced via', severity, vuln.type === 'license' ? 'issue' : 'vuln', vulnIn, from);
            messageIntro = createSeverityBasedIssueHeading(messageIntro, vuln.severity);
